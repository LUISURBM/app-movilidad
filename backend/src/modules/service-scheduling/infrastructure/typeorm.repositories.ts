...
export class TypeOrmServicioRepository implements ServicioRepository {
  ...
  private conRepo<T>(tenant: TenantId, work: (repo: Repository<ServicioEntity>) => Promise<T>): Promise<T> {
    return enTenant(this.dataSource, tenant, (m) => work(m.getRepository(ServicioEntity)));
  }
